...
// ======================================================================
// Tests for the add_constrained_err(int a, int b, int &result) function
// ======================================================================

/**
 * @test Happy Path: Verifies that valid inputs within bounds return ESP_OK.
 */
TEST(TestSum, AddConstrainedErr_HappyPath)
{
    Sum calc;
 ...
 * @test Edge Cases: Verifies behavior at the exact boundaries of the 0-10 range.
 */
TEST(TestSum, AddConstrainedErr_EdgeCases)
{
    Sum calc;

    // Minimum boundary: 0 + 0 should be 0
    int result = 0;
    esp_err_t err = calc.add_constrained_err(0, 0, result);
    EXPECT_EQ(err, ESP_OK);
    EXPECT_EQ(result, 0);
    ...
    // Maximum single input: 10 + 0 should be 10
    err = calc.add_constrained_err(10, 0, result);
    EXPECT_EQ(err, ESP_OK);
    EXPECT_EQ(result, 10);
}

/**
 * @test Out of Range: Verifies that inputs or results exceeding limits return ESP_FAIL.
 */
TEST(TestSum, AddConstrainedErr_OutOfRange)
{
    Sum calc;

    // Input above the limit of 10. invlaid inputs return ESP_ERR_INVALID_ARG
    int result = 0;
    esp_err_t err = calc.add_constrained_err(11, 0, result);
    EXPECT_EQ(err, ESP_ERR_INVALID_ARG);

    // Negative input below 0
    err = calc.add_constrained_err(-1, 5, result);
    EXPECT_EQ(err, ESP_ERR_INVALID_ARG);

    // Result exceeds 10 despite valid individual inputs (6 + 5 = 11), invalid results return ESP_FAIL
